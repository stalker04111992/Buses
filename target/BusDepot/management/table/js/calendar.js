...
-function () {
	...
	lang = {
		...
		mm:['Янв', 'Фев', 'Мар', 'Апр', 'Май', 'Июнь', 'Июль', 'Авг', 'Сен', 'Окт', 'Ноя', 'Дек'],
	...
	var setDate = function (e) {
		...
		this.input.date = new Date(this.year, this.month, parseInt(d));
		var date = new Date(this.year, this.month, parseInt(d) + 1);
		var val = date.toISOString();//lang.formatDate(this.input.date);
		this.input.value = val.substring(0, val.indexOf('T'));
		cc(this, 'disnone');
	...
	var buildField = function (el1, params, idxs) {

		var el = ce(el1), name = params.name;
