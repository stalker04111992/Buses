...
-function () {
	...
	var setDate = function (e) {
		...
		this.input.date = new Date(this.year, this.month, parseInt(d) + 1);
		this.input.value = (this.input.date).toISOString();//lang.formatDate(this.input.date);
		this.input.value = this.input.value.substring(0, this.input.value.indexOf('T'));
		cc(this, 'disnone');
